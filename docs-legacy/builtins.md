...
- Input: `int` (port)
- Output: `int!` (Result containing the socket file descriptor)

### `tcpAccept(fd)`
Accepts an incoming connection on a listening socket.
- Input: `int` (listen file descriptor)
- Output: `int!` (Result containing the client socket file descriptor)

### `tcpRecv(fd, max_len)`
Receives data from a TCP socket.
- Input: `int` (fd), `int` (maximum bytes to read)
- Output: `str!` (Result containing the received string)

### `tcpSend(fd, data)`
Sends string data over a TCP socket.
- Input: `int` (fd), `str` (data)
- Output: `int!` (Result containing the number of bytes sent)
...
### `httpParse(raw_request)`
Optimized native function to parse a raw HTTP request string.
- Input: `str`
- Output: `{str:any}!` (Result containing a map with "method", "path", "headers", and "body")

### `httpFormat(response_map)`
Optimized native function to format an HTTP response map into a string.
- Input: `{str:any}` (Map with "status", "headers", and "body")
- Output: `str`
